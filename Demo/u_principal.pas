...
type
  TfrmPrincipal = class(TForm)
    ...
    procedure btSendQuotedClick(Sender: TObject);
...
//# Gabriel Rocha
procedure TfrmPrincipal.btSendQuotedClick(Sender: TObject);
begin
  try
    if not TInject1.Auth then
       Exit;

    TInject1.sendQuoted(ed_num.Text, mem_message.Text, ed_IDMessage.Text);
  finally
    ed_num.SelectAll;
    ed_num.SetFocus;
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            //Gabriel Rocha (
            memo_unReadMessage.Lines.Add(PChar( 'Message Id     : ' + AMessage.id));
            if AMessage.quotedMsg <> Nil then
            begin
              memo_unReadMessage.Lines.Add(PChar( 'Quoted Message ID     : ' + AMessage.quotedStanzaID));
              memo_unReadMessage.Lines.Add(PChar( 'Quoted Message Participant    : ' + AMessage.quotedParticipant));
              memo_unReadMessage.Lines.Add(PChar( 'Quoted Message     : ' + AMessage.quotedMsg.Body));
            end;
            //)
            FChatID := AChat.id;
          ...
          begin
            memo_unReadMessage.Lines.Add('Mensagem Enviada:');
            memo_unReadMessage.Lines.Add(PChar( 'Message Id     : ' + AMessage.id));
            if AMessage.quotedMsg <> Nil then
            begin
              memo_unReadMessage.Lines.Add(PChar( 'Quoted Message ID     : ' + AMessage.quotedStanzaID));
